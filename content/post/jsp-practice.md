...
class MyServlet1 : HttpServlet() {     生成
    ...
    override fun doGet(request: HttpServletRequest, response: HttpServletResponse) {
        val image = BufferedImage(70, 25, BufferedImage.TYPE_INT_RGB)
        val graphics = image.graphics.apply {
            this.font = Font("Microsoft YaHei UI", Font.PLAIN, 18)     设置字体
            this.color = Color(200, 150, 255)     设置画笔颜色
...
class MyServlet2 : HttpServlet() {     验证
    override fun doPost(request: HttpServletRequest, response: HttpServletResponse) {
        ...
        if (temp1 == temp2) {     对接收到的验证码和 Session 中的验证码进行比较
